...
YARP_LOG_COMPONENT(GPTTRANSCRIBEDEVICE, "yarp.gptTranscribeDevice", yarp::os::Log::TraceType);


GptTranscribeDevice::GptTranscribeDevice()
{

}

bool GptTranscribeDevice::open(yarp::os::Searchable &config)
{
    if (!parseParams(config))  { return false; }

    if(std::getenv(m_ENVS_api_key_name.c_str()) == nullptr)
    {
        yCError(GPTTRANSCRIBEDEVICE) << "Environment variable" << m_ENVS_api_key_name << "not set";
        return false;
    }
    if(std::getenv(m_ENVS_end_point_name.c_str()) == nullptr)
    {
        yCError(GPTTRANSCRIBEDEVICE) << "Environment variable" << m_ENVS_end_point_name << "not set";
        return false;
    }
    if(std::getenv(m_ENVS_deployment_id_name.c_str()) == nullptr)
    {
        yCError(GPTTRANSCRIBEDEVICE) << "Environment variable" << m_ENVS_deployment_id_name << "not set";
        return false;
    }
    if(std::getenv(m_ENVS_api_version_name.c_str()) == nullptr)
    {
        yCError(GPTTRANSCRIBEDEVICE) << "Environment variable" << m_ENVS_api_version_name << "not set";
        return false;
    }
    ...
    yCInfo(GPTTRANSCRIBEDEVICE) << "Open";
    return true;
}

bool GptTranscribeDevice::close()
{
    yCInfo(GPTTRANSCRIBEDEVICE) << "Close";
    return true;
}

ReturnValue GptTranscribeDevice::setLanguage(const std::string& language)
{
    yCWarning(GPTTRANSCRIBEDEVICE) << "setLanguage not implemented";
    return YARP_METHOD_NOT_YET_IMPLEMENTED();
}

ReturnValue GptTranscribeDevice::getLanguage(std::string& language)
{
    yCWarning(GPTTRANSCRIBEDEVICE) << "getLanguage not implemented";
    return YARP_METHOD_NOT_YET_IMPLEMENTED();
}

ReturnValue GptTranscribeDevice::transcribe(const yarp::sig::Sound& sound, std::string& transcription, double& score)
{
    score = 0.0;
    CURL *curl = curl_easy_init();
    if (!curl) {
        yCError(GPTTRANSCRIBEDEVICE) << "Failed to initialize cURL";
    ...
    if (res != CURLE_OK) {
        yCError(GPTTRANSCRIBEDEVICE) << "cURL request failed: " << curl_easy_strerror(res);
    } else {
        yCDebug(GPTTRANSCRIBEDEVICE) << "Transcription response: " << response;
    }
    ...
    try {
        ...
        if (jsonResponse.contains("text")) {
            ...
            yCError(GPTTRANSCRIBEDEVICE) << "No 'text' field in the response";
        ...
        if (jsonResponse.contains("segments") && !jsonResponse["segments"].empty()) {
            if (jsonResponse["segments"][0].contains("avg_logprob") && jsonResponse["segments"][0].contains("no_speech_prob")) {
                ...
                yCWarning(GPTTRANSCRIBEDEVICE) << "No 'avg_logprob' or 'no_speech_prob' field in the first segment, setting score to 0.0";
                score = 1.0;
            }
            ...
            yCWarning(GPTTRANSCRIBEDEVICE) << "No 'confidence' field in the response, setting score to 0.0";
            score = 0.0;
        }
    } catch (const nlohmann::json::parse_error& e) {
        yCError(GPTTRANSCRIBEDEVICE) << "JSON parse error: " << e.what();
        return ReturnValue::return_code::return_value_error_generic;
    }
...
std::vector<uint8_t> GptTranscribeDevice::_createWavHeader(int sampleRate, int numSamples)
{
...
size_t GptTranscribeDevice::_writeCallback(void *contents, size_t size, size_t nmemb, std::string *output) {
